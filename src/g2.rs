...
impl groupy::CurveProjective for G2Projective {
    ...
    fn batch_normalization<S: std::borrow::BorrowMut<Self>>(v: &mut [S]) {
        // Simultaneously invert a vector of _non-zero_ elements of Fp2 using
        // Montgomery's inversion trick
        fn batch_inverse(inputs: &mut [Fp2]) {
            let mut scratch = vec![Fp2::zero(); inputs.len()];
            let mut acc = Fp2::one();
